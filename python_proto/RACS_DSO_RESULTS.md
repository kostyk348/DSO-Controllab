...
### Priority 1: Full GP convergence ⏳ частично
pop=80/gens=30 выполнено (раздел 5). pop=200/gens=60 запускался, но Python-симуляции
слишком медленные (~50+ мин) — нужен векторизованный симулятор или C-эвалюатор для финального прогона.

---

**Статус проекта:** все основные направления DSO реализованы и валидированы:
- ✅ синтез (GP → NSGA-II, variance-centric)
- ✅ ресурсные контракты + memory planner
- ✅ codegen + калиброванная cycle-модель (±7% vs QEMU)
- ✅ hardware-in-the-loop (IAE ±0–5.4%)
- ✅ streaming compiler (multi-rate: IAE −30%, CPU −52%)
- ✅ on-target Bayesian optimization

**Рекомендация на будущее:** (1) реальная плата STM32F4 вместо QEMU; (2) векторизованный
симулятор для pop=200+; (3) knee-point/hypervolume selection; (4) многоскоростной
GP-синтез (синтезировать сразу весь pipeline, а не отдельные регуляторы).
